...
@section('content')
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">{{trans('games.add_new')}}</li>
    </ol>
  </section>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-warning">
          ...
          <form method="POST" id="games" action="{{route('games.store')}}" accept-charset="UTF-8" enctype="multipart/form-data">
            @csrf
            <div class="box-body">
                ...
                @foreach(config('app.locales') as $lk=>$lv)
                  <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
                    <div class="form-group">
                      <label for="level_name:{{$lk}}" class="content-label">{{trans('games.name')}}</label>
                      <input class="form-control" minlength="2" maxlength="255" placeholder="{{trans('games.name')}}" name="game_name:{{$lk}}" type="text" value="{{old('name:{$lk}')}}">
