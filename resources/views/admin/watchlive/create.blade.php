    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-warning">
          ...
          <form method="POST" id="watchlive" action="{{route('watch_live.store')}}" accept-charset="UTF-8" enctype="multipart/form-data">
            @csrf
            <div class="box-body">
              <div class="row">
                <div class="col-lg-6">
						...
						@foreach(config('app.locales') as $lk=>$lv)
						  <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
								<div class="form-group">
									 ...
									 <input type="text" class="form-control" name="match_name:{{$lk}}" placeholder="{{trans('watchlive.match_name')}}" value="">
								</div>
							</div>
						@endforeach
                    </div>  
					 ...
					 <div class="row">
						 <div class="col-lg-6">
							<div class="form-group">
							  ...
							  <input type="text" class="form-control"  name="price"  placeholder="{{trans('watchlive.price')}}" value="">
							</div>
						 </div>
						 <div class="col-lg-6">
							<div class="form-group">
							  <label for="currency" class="content-label">{{trans('watchlive.currency')}}</label>
							  <input type="text" class="form-control" name="currency"  placeholder="{{trans('watchlive.currency')}}" value="">
							</div>
						 </div>
					</div>
					<div class="form-group">
						<label for="online_link" class="content-label">{{trans('watchlive.online_link')}}</label>
						<input type="text" class="form-control" name="online_link" placeholder="{{trans('watchlive.online_link')}}" value="">
					</div>
                </div>
              </div>
