    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-warning">
          ...
          <form method="POST" id="levels" action="{{route('levels.update', $level->id)}}" accept-charset="UTF-8">
            @csrf
              ...
              <div class="row">
                    ...
                    @foreach(config('app.locales') as $lk=>$lv)
                      <div role="tabpanel" class="tab-pane @if($lk=='en') active @endif" id="abc_{{$lk}}">
                        <div class="form-group">
                          ...
                          <input class="form-control" minlength="2" maxlength="255" placeholder="{{trans('levels.level_name')}}" name="level_name:{{$lk}}" type="text" value="{{$level->translate($lk)->level_name}}">
                ...
                <div class="col-lg-6">
                  <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="no_of_question" value="{{$level->no_of_question}}">
                  </div>
                  <div class="form-group">
                    <label for="plus_point_per_que" class="content-label">{{trans('levels.plus_point_per_que')}}</label>
                    <input type="number" class="form-control" name="plus_point_per_que" value="{{$level->plus_point_per_que}}">
                  </div>
                  <div class="form-group">
                    <label for="minus_point_per_que" class="content-label">{{trans('levels.minus_point_per_que')}}</label>
                    <input type="number" class="form-control" name="minus_point_per_que" value="{{$level->minus_point_per_que}}">
                  </div>
                </div>
              </div>
            </div>
